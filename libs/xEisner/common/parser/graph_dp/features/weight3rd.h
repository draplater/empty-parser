#ifndef _WEIGHT_3RD_H
#define _WEIGHT_3RD_H
...
class Weight3rd : public WeightBase {
public:
	Token * m_pWords;
	Token * m_pPOSTags;
...
public:
	Weight3rd(const std::string & sRead, const std::string & sRecord, int nParserState, Token * words, Token * postags);
	~Weight3rd();
	...
	tscore getOrUpdateTriArcScore(const int & p, const int & c, const int & c2, const int & c3, const int & amount, int sentLen, WordPOSTag(&sent)[MAX_SENTENCE_SIZE]);
};
