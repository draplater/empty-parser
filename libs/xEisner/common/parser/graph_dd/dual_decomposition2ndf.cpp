#include "dual_decomposition2ndf.h"

void DualDecomposition2ndf::averageScore(const DependencyTree & tree, bool empty) {
...
void DualDecomposition2ndf::init(const std::string & eisner2ndModel, const std::string & emptyeisner2ndModel, tscore step, int maxIter) {
	m_vecSteps.clear();
	...
	m_pEmptyEisnerSecondParser.reset(new emptyeisner2ndf::DepParser(emptyeisner2ndModel, emptyeisner2ndModel, ParserState::PARSE));
...
void DualDecomposition2ndf::decode(const DependencyTree &eisner2ndTree, const DependencyTree &emptyeisner2ndTree, DependencyTree &ret_tree, float lambda) {
...
bool DualDecomposition2ndf::isCoordinate(const DependencyTree & eisner2ndTree, const DependencyTree & emptyeisner2ndTree, int iteration) {
	bool is_coord = true;
